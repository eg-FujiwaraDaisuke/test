...
void main() {
  ...
  setUp(() {
    mockHttpClient = MockHttpClient();
    dataSource = TokenDataSourceImpl(client: mockHttpClient);
  });
  ...
  group('getToken', () {
    ...
    final tBody = json.encode({'refreshToken': tRefreshToken});
    const tUrl = 'https://www.dev.minapp.minden.co.jp/api/v1/auth/renewToken';
    final tTokenModel =
    ...
    test('should preform a Post request', () async {
      ...
      await dataSource.getToken(tRefreshToken);
      verify(
        mockHttpClient.post(
          tUrl,
    ...
    test('response code is 200', () async {
      when(mockHttpClient.post(
        ...
        (_) async => http.Response(fixture('token_data.json'), 200),
      );
      final result = await dataSource.getToken(tRefreshToken);
